...
const CountryDropdown = () => {
    ...
    return (
        <>
            <Button className='countryDrop bg-blue' onClick={() => {
                setisOpenModal(true);
                setcountryList(context.countryList);
            }}>
                <div className='info d-flex flex-column'>
                    <span style={{color:"white"}}>Your Location</span>
                    <span style={{color:"white"}}>{context.selectedCountry !== "" ? context.selectedCountry.length > 10 ? context.selectedCountry?.substr(0, 10) + '...' : context.selectedCountry : 'Select Location'}</span>
                </div>
